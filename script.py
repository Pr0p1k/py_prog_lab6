...
config = {
    'sqlite': {
        'driver': 'sqlite',
        'database': '/Users/aleksandr.prokopyev/PycharmProjects/pythonProject/lab6/pyth',
        'prefix': ''
    }
}
...
def get_feed_page(feedname, pagesize, pagenum):
    query = db.table('rss').where('feed', '=', feedname).order_by('published', 'desc').skip(pagenum * pagesize).take(
        pagesize)
    return query.get()
...
def load_entries(url):
    res = feedparser.parse(url.strip())
    feed = res['feed']['title']
    for entry in res['entries']:
        ...
        #     tags = tags + ', ' + tg['term']
        ent = RssEntry(title, id, link, text, '', published, feed, url, tags)
        ent.save()
    return feed
